"""
Snowflake Dashboard - Find by Integration Type
================================================
Uses content.oauth.associations.find_by(integration_type=...) to discover
the Snowflake integration GUID by type.
...
app_ui = ui.page_sidebar(
    ...
    title="Snowflake Analytics (find by type)",
)
...
def server(i: Inputs, o: Outputs, session: Session):
    ...
    def load_all():
        ...
        if sf_assoc is None:
            ui.notification_show(
                "No Snowflake integration found on this content", type="error"
            )
            return
