"""
Databricks Dashboard - Find by Integration Type
=================================================
Uses content.oauth.associations.find_by(integration_type=...) to discover
the Databricks integration GUID by type.
...
app_ui = ui.page_sidebar(
    ...
    title="Databricks Analytics (find by type)",
)
...
def server(i: Inputs, o: Outputs, session: Session):
    ...
    def load_all():
        ...
        if db_assoc is None:
            ui.notification_show(
                "No Databricks integration found on this content", type="error"
            )
            return
