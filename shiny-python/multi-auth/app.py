"""
Multi-Source Dashboard - Find by Integration Type
===================================================
Uses content.oauth.associations.find_by(integration_type=...) to discover
both Snowflake and Databricks integration GUIDs by type.
...
app_ui = ui.page_sidebar(
    ...
    title="Multi-Source Analytics (find by type)",
)
...
def server(i: Inputs, o: Outputs, session: Session):
    ...
    def load_all():
        ...
        if sf_assoc is None or db_assoc is None:
            ui.notification_show(
                f"Missing: snowflake={sf_assoc is not None}, "
                f"databricks={db_assoc is not None}",
                type="error",
            )
            return
